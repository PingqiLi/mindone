...
class ClapAudioSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in ClapAudioModel construct() function)
...
class ClapTextSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in ClapTextModel construct() function)
...
class ClapModel(ClapPreTrainedModel):
    ...
    def __init__(self, config: ClapConfig):
        ...
        self.logit_scale_a = Parameter(mindspore.tensor(math.log(config.logit_scale_init_value)))
        self.logit_scale_t = Parameter(mindspore.tensor(math.log(config.logit_scale_init_value)))
    ...
    def construct(
        ...
        >>> probs = logits_per_audio.softmax(axis=-1)  # we can take the softmax to get the label probabilities
        ```"""
        # Use CLAP model's config for some fields (if specified) instead of those of audio & text components.
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
        ...
        # normalized features
        audio_embeds = audio_embeds / ops.norm(audio_embeds, ord=2, dim=-1, keepdim=True)
