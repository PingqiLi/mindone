...
from ...mindspore_utils import weight_norm
...
def _compute_mask_indices(
    ...
    for input_length in input_lengths:
        ...
        if len(spec_aug_mask_idx) == 0:
            # this case can only happen if `input_length` is strictly smaller than
...
class HubertEncoderLayer(nn.Cell):
    ...
    This class inherits from nn.Cell and contains methods to initialize the layer and forward the layer.
...
class HubertAttnAdapterLayer(nn.Cell):
    ...
    This class inherits from nn.Cell and provides functionality to forward adapter modules directly with 3D tensor
...
class HubertEncoderLayerStableLayerNorm(nn.Cell):
    ...
    This class inherits from nn.Cell and contains methods for initializing the layer and forwarding the layer
...
class HubertEncoder(nn.Cell):
    ...
    def construct(
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask * float(np.finfo(hidden_states.dtype).min)
...
# Copied from transformers.models.wav2vec2.modeling_wav2vec2.Wav2Vec2ForCTC
# with Wav2Vec2->Hubert, wav2vec2->hubert, WAV_2_VEC_2->HUBERT
class HubertForCTC(HubertPreTrainedModel):
    ...
    def tie_weights(self):
        """
        This method overwrites [`~MSPreTrainedModel.tie_weights`] so that adapter weights can be correctly loaded when
        passing `target_lang=...` to `from_pretrained(...)`.

        This method is **not** supposed to be called by the user and is prone to be changed in the future.
        """
        # Note that `tie_weights` is usually used to tie input and output embedding weights. The method is
        # re-purposed to correctly load adapter layers for Hubert so that we do not have to introduce a new API to [
        # `MSPreTrainedModel`]. While slightly hacky, Hubert never has to tie input and output embeddings,
